...
```bash
npm install -g ./clawdbot-cn-2026.1.25.tgz
```
...
```bash
# 查看包信息
npm view clawdbot-cn

# 安装测试
npm install -g clawdbot-cn
...
### 1. 发布失败：包名已存在
确保包名 `clawdbot-cn` 在 npm 上可用。如果已被占用，需要更换包名。
...
发布后 72 小时内可以撤销：
```bash
npm unpublish clawdbot-cn@2026.1.25
```
