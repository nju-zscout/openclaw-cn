...
- `moltbot-cn models status`: checks provider auth + model availability.
...
If you don’t have a global install yet, run it via `pnpm moltbot-cn onboard`.
...
`moltbot-cn onboard` is the recommended setup path. In **local mode** it walks you through:
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `moltbot-cn models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `moltbot-cn models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).
...
Steps:
1) Enable the plugin: `moltbot-cn plugins enable google-gemini-cli-auth`
2) Login: `moltbot-cn models auth login --provider google-gemini-cli --set-default`
...
- Channel health: `moltbot-cn channels status`
...
`moltbot-cn models status` reports whether **shell env import** is enabled. “Shell env: off”
...
```bash
moltbot-cn directory groups list --channel whatsapp
```
...
- `moltbot-cn models set ...` (updates just model config)
...
- **Sanity‑check model/auth status**
  - Use `moltbot-cn models status` to see configured models and whether providers are authenticated.
...
- **Sync the Claude Code CLI token on the gateway host**
  - Run `moltbot-cn models status` (it loads + syncs Claude Code CLI credentials).
  - If it still says missing: run `claude setup-token` (or `moltbot-cn models auth setup-token --provider anthropic`) and retry.
- **If the token was created on another machine**
  - Paste it into the gateway host with `moltbot-cn models auth paste-token --provider anthropic`.
...
Clawdbot may temporarily skip a profile if it’s in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `moltbot-cn models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
moltbot-cn models auth order set --provider anthropic anthropic:claude-cli anthropic:default
...
```bash
moltbot-cn models auth order set --provider anthropic --agent main anthropic:claude-cli
```
...
```bash
moltbot-cn channels status
moltbot-cn channels logs --channel telegram
```
...
```bash
moltbot-cn message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
- Default behavior on DM‑capable channels is **pairing**:
  ...
  - Approve with: `moltbot-cn pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `moltbot-cn pairing list <channel>` if a code didn’t arrive.
