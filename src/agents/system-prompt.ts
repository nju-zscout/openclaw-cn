...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "When diagnosing issues, run `openclaw-cn status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "If unsure, ask the user to run `openclaw-cn help` (or `openclaw-cn gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
